...
function elgg_generate_plugin_entities() {
	...
	//echo var_dump($known_plugins)."<br/>";
	...
	//echo "plugins after removing invalid ids<br/>";
	//echo var_dump($known_plugins);
...
//	echo var_dump($physical_plugins);
//	if(true){return true;}
	//echo "In DB<br/>";
	...
	//	if ($plugin->isActive()) {
			//echo "Deactivated ".($plugin)."<br/>";
	//		$plugin->deactivate();
	//	}
		//echo "Disable ".($plugin->title)."<br/>";
		// remove the priority.

	//	$name = elgg_namespace_plugin_private_setting('internal', 'priority');
	//	echo $plugin->guid." ".$plugin->title." ".$name." ";	
	//	remove_private_setting($plugin->guid, $name);
//	echo "Removed private setting <br/>";
//HERES THE PROBLEM TRYING TO DISABLE PLUGIN
		//$disableEntity = get_entity($plugin->guid);
		//echo var_dump($disableEntity);
		//echo var_dump($e)."<br/>";;
		//$disableEntity->disable();	
		//error_log("Trying to disable ".$plugin_guid." ".$plugin->title);	
//		$plugin->disable();
//$result = $disableEntity->getDatabase()->updateData("UPDATE {$CONFIG->dbprefix}entities SET enabled = 'yes' WHERE guid = $disableEntity->guid");
//		echo $result;
		//error_log("Disabled ".$plugin->title);

		//echo $disableEntity->guid." ".$disableEntity->title." is disabled <br/>";
	//}
	//if(true){return true;}
...
function remove_stale_plugins(){
 ...
 // check real plugins against known ones
      ...
      foreach($known_plugins as $p){
                $guid_array[] = $p->guid;
                //echo $p->enabled." ". $p->title." ".$p->guid."<br/>";
                if($p->isActive()){
                        $p->deactivate();
                }
                //echo "private setting";
                $name =  elgg_namespace_plugin_private_setting('internal', 'priority');
                remove_private_setting($plugin->guid, $name);

                $plugin->disableMetadata();
                $plugin->disableAnnotations();
                //too slow, changed to single update
                //query below
                //$p->disable();
 }

//run single update query with all guids
        $string_guids = implode(",",$guid_array);


  $query ="UPDATE {$db_prefix}entities
                SET enabled = 'no'
                WHERE guid in ({$string_guids})
                ";
                echo $query;
                $res = get_data($query);
