(function () {
    ...
    function Projects(resolveProject, project, user, notification, $location, Upload, $routeParams, $rootScope, helper, DTOptionsBuilder, $q, $sce, $uibModal, $filter) {
        tinyMCE.remove();
	...
	vm.selected = null;
        ...
        //filter object
        vm.filters = {owner_guid: '', status: '', project_type: '', department_owner: '', archived: ''};
	...
	vm.filters.archived = 'false';
        ...
        vm.statuses = [
            {name: elgg.echo('projects:label:submitted'), id: "Submitted"},
            {name: elgg.echo('projects:label:underreview'), id:"Under Review",},
            {name: elgg.echo('projects:label:inprogress'), id:"In Progress"},
            {name: elgg.echo('projects:label:completed'), id: "Completed"},
            {name: elgg.echo('projects:label:cancelled'), id: "Cancelled"}
        ];
	    ...
	    getSpaceInConfluence(vm);
            ...
            //getProjects();
	    filterProjects('Project','classification'); 
	...
	function filterProjects(id, dataFilterType){

            var filter = id;
            var filterType = dataFilterType;
            //Testing
...
vm.createTask = function (isValid) {
	...
	vm.archiveProject = function (id){		
          Upload.upload({
              url: 'internapi/projects',
              data: {'projectId': id, 'action': 'archiveProject'}
          }).then(function (success) {
             // window.location.reload();
                          //  $location.path('projects');
getProjectsByParam(vm.filters).then(function (success) {
	...
	vm.unarchiveProject = function (id){		
          Upload.upload({
              url: 'internapi/projects',
              data: {'projectId': id, 'action': 'unarchiveProject'}
          }).then(function (success) {
             // window.location.reload();
                          //  $location.path('projects');
		getProjectsByParam(vm.filters).then(function (success) {
...
//return if the space exisits in confluence
 ...
 //get all the spaces without Project Charter
		...
		baseUrl = (baseUrl.slice(-1) == "/") ? baseUrl : baseUrl+"/";
            ...
            //MPC LSC OPI / Manager
            projCharterPg2 += "<td style='width:30%;'>" + "MPC LSC OPI/Manager" + "</td>" +
